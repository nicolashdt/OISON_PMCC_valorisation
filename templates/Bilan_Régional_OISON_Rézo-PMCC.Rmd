...
colors_statut <- c(
  "envahissante" = "#F77F00",   # Orange
  "menacée" = "#3DAF2A",        # Vert clair
...
## Evolution de la saisie moyenne par observateur
...
saisies_observateur_histo <- saisies_observateur
saisies_observateur_histo$annee <- as.factor(saisies_observateur_histo$annee)

saisies_observateur_derniere_annee <- OISON_PMC %>%
  st_set_geometry(NULL) %>%
  filter(annee == params$annee_fin) %>%
  group_by(annee, INSEE_DEP, Observateur) %>%
  summarise(nombre_saisies = n()) %>%
...
saisies_observateur_tableau <- saisies_observateur %>%
  mutate(moyenne_saisies_observateur=round(moyenne_saisies_observateur,0)) %>%  # arrondi les moyennes
...
flextable(saisies_observateur_tableau) %>%
  ...
  align(j = 2:7, align = "right") %>%   # Aligner à droite les colonnes de saisies
  align(part = "header",j = 2:7, align = "right") %>% 
  set_caption(caption = paste("Evolution de la saisie moyenne par observateur par département",nom_bases))
...
ggplot(saisies_observateur) +
  ...
  labs(
    ...
    color = "Départements",
    title = paste("Evolution de la saisie moyenne par observateur", nom_bases)
  ) +
...
ggplot(saisies_observateur_violon, aes(x = INSEE_DEP, y = nombre_saisies, fill=INSEE_DEP)) +
  ...
  labs(
    ...
    title = paste("Répartition du nombre de saisie par observateur en",params$annee_fin, nom_bases),
    caption="Explications :
    - La largeur du violon indique le nombre d'observateurs relatif
...
## Evolution régionale du nombre de saisies par groupe d'espèces
...
ggplot(groupes_espece_par_annee) +
  ...
  labs(x = "Années",
       ...
       title = paste("Evolution régionale du nombre de saisies par groupe d'espèce",nom_bases)) +
  theme_minimal() +
  theme(text = element_text(size = 9))
```
...
ggplot(groupes_espece)+
  ...
  geom_text(data=tail(groupes_espece,3),
            aes(x = 1.8, 
                ...
                size = 3)+
  labs(
    title = paste("Proportions du nombre de saisies régionales par groupe d'espèce en",params$annee_fin,nom_bases),
...
flextable(groupes_espece_tableau) %>%
  ...
  set_caption(caption = paste("Nombre de saisie par groupe d'espèces et par échelle en", params$annee_fin ,nom_bases)) %>% 
...
if (params$OISON) {
  cat("\\newpage\n")
  cat("## Evolution du nombre de saisies régionales par groupe d'espèces (sans oiseaux)\n")
}
```
...
if (params$OISON) {
  ggplot(groupes_espece_par_annee_sans_oiseaux) +
    ...
    labs(x = "Années",
         y = "Nombre de saisies",
         colour = "Groupes d'espèce",
         title = paste("Evolution du nombre de saisies régionales par groupe d'espèces sans les oiseaux", nom_bases)) +
    theme_minimal() +
    theme(text = element_text(size = 9))
}
```
...
if (params$OISON) {
  ggplot(groupes_espece_sans_oiseaux) +
    ...
    labs(
      ...
      title = paste("Proportions des saisies régionales par groupe d'espèces sans les oiseaux en", params$annee_fin, nom_bases)
    ) +
  theme(
    text = element_text(size = 9),
    plot.title = element_text(hjust = 0.2),  # Aligne le titre à gauche
...
## Proportions d'espèces sans les oiseaux par département

<br> Graphiques radars sur les proportions (%) d'espèces saisies sans les oiseaux de `r params$annee_debut` à `r params$annee_fin` : <br>
...
## Evolution régionale du nombre de saisies par statut
...
flextable(statuts_espece_par_annee_tableau) %>%
  ...
  set_caption(caption = paste("Evolution régionale du nombre de saisies par statut" ,nom_bases)) %>% 
  set_table_properties(width = 0.8, layout = "autofit")
...
ggplot(statuts_espece_par_annee) +
  ...
  labs(x = "Années",
       y = "Nombre de saisies",
       colour = "Statuts",
       title = paste("Evolution régionale du nombre de saisies par statut", nom_bases)) +
  theme_minimal() +
  theme(text = element_text(size = 9))
```
...
ggplot(statuts_espece)+
  ...
  labs(
    fill = "Statuts",
    title = paste("Proportions du nombre de saisies régionales par statut en",params$annee_fin,nom_bases)
    ) +
  theme(
    text = element_text(size = 9),
    plot.title = element_text(hjust = 0.2),  # Aligne le titre à gauche
...
ggplot(statuts_espece_dep) +
  ...
  labs(
    ...
    y = "Nombre de saisies",
    fill = "Départements",
    title = paste("Nombre de saisie par statut par département en", params$annee_fin, nom_bases)) +
  theme_minimal() +
  theme(text = element_text(size = 9))
```
...
# Evolutions sur les espèces protégées
...
flextable(espece_pro_par_annee_tableau) %>%
  ...
  set_caption(caption = paste("Evolution régionale du nombre de saisies des espèces protégées" ,nom_bases)) %>% 
...
flextable(groupes_espece_pro_tableau) %>%
  ...
  align(j = 2:13, align = "right") %>%   # Aligner à droite les colonnes de saisies
  align(part = "header",j = 2:13, align = "right") %>% 
  set_caption(caption = paste("Nombre de saisie par groupe d'espèces protégées et par échelle en", params$annee_fin ,nom_bases)) %>% 
  set_table_properties(width = 0.8, layout = "autofit")
...
ggplot(espece_pro_par_annee) +
  ...
  labs(x = "Années",
       ...
       title = paste("Evolution régionale du nombre de saisies des espèces protégées", nom_bases)) +
  theme_minimal() +
  theme(text = element_text(size = 9))
```
...
ggplot(espece_pro_dep) +
  ...
  labs(
    ...
    y = "Nombre de saisies",
    fill = "Départements",
    title = paste("Nombre de saisie par réglementation et par département en", params$annee_fin, nom_bases)) +
  theme_minimal() +
  theme(text = element_text(size = 9))
```
...
ggplot(groupes_espece_pro)+
  ...
  geom_text(data=tail(groupes_espece_pro,2), #CHANGER LE CHIFFRE POUR AFFICHER =/- de NOMS
            aes(x = 1.8, 
                ...
                size = 3)+
  labs(
    title = paste("Nombre de saisies régionales par groupe d'espèce protégées en", params$annee_fin, nom_bases),
    fill = "Groupes d'espèces"
    ) +
  theme(
    text = element_text(size = 9),
...
ggplot(observations_espece_non_pro) +
  ...
  geom_sf(data = departements, fill = NA, color = "black", lwd = 0.5) +
  scale_color_manual(values = colors_pro, name = "Réglementation") +
  labs(title = paste("Saisies des espèces non protégées de",params$annee_debut,"à",params$annee_fin,nom_bases)) +
  theme_void() +
  theme(text = element_text(size = 9))
```
