...
observateurs_dep <- OISON_PMC %>% 
  ...
  summarise(nombre_observateurs_dep = n_distinct(Observateur))
...
```{r}
saisies_observateur_dep <- OISON_PMC %>%
  st_set_geometry(NULL) %>%
  filter(INSEE_DEP==params$dep) %>% 
  filter(annee >= params$annee_debut & annee <= params$annee_fin) %>%
  ...
  group_by(annee, INSEE_DEP) %>%
  summarise(moyenne_saisies_observateur_dep = round(mean(nombre_saisies),1))
